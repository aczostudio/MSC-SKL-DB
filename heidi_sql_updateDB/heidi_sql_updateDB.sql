...
CREATE TABLE IF NOT EXISTS `customerproductorder` (
  ...
  `CustomerProductOrderDate` varchar(50) DEFAULT NULL,
  ...
  PRIMARY KEY (`CustomerProductOrderID`),
  KEY `FOREIGN KEY` (`ProductName`,`ClerkID`,`CustomerName`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='Customer''s product order';

-- Dumping data for table msc-skl_db.customerproductorder: ~0 rows (approximately)
...
INSERT INTO `customerproductorder` (`CustomerProductOrderID`, `CustomerProductOrderDate`, `ProductName`, `CustomerProductOrderQuantity`, `CustomerProductOrderVat`, `ClerkID`, `CustomerName`) VALUES
	('co_dummy_id', '12/07/2019', 'BA-CYLN-HWAYAN', 3, 0.07, 'clerk_dummy', 'customer_dummy');
...
-- Dumping data for table msc-skl_db.product: ~4 rows (approximately)
DELETE FROM `product`;
